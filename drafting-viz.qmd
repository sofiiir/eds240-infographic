...
#### Visual mockup 
![feeding_mock_up](final_visual_draft/bat_feeding_mockup.jpg)
...
Define colors
```{r}
feeding_pal <- c("Carnivore" = "#E6AA60",
                 "Sanguivore" = "#883955",
                 "Frugivore" = "#C4EBC8",
                 "Insectivore" = "#587792",
                 "Nectarivore" = "#D7A6B3",
...
```{r}
feedinggroup <- bat_feeding_groups |> 
  arrange(value) |> 
ggplot(aes(fill= fct_inorder(feeding_groups), y=value, x = "")) +
  geom_bar(position="fill", stat="identity") +
  scale_fill_manual(values = feeding_pal)+
  labs(y = "Percentage")+
  theme_minimal() + 
...
# save 14"x12" png ---
ggsave(
  filename = here::here("final_visual_draft", "feedinggroup_bp.png"),
  plot = feedinggroup,
  device = "png",
  width = 14, 
  height = 12,
...
```
The code below is where I am attempting to figure out how to make a matrix to create the stacked barplot.
```{r}
...
The matrix is extremely confusing. I used affinity to fit the stacked barplot into the bat but am deeply unsatisified. If I don't figure out how to make the matrix I will look for another type of plot. 
![batfeedinggroups](final_visual_draft/batfeedinggroups.png)
