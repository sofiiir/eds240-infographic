...
```{r}
# bat_geo1 <- st_as_sf(geo1, coords = c("lattitude", "longitude"), crs = 4326)
# bat_geo2 <- st_as_sf(geo2, coords = c("latitude", "longitude"), crs = 4326)
bat_geo <- st_as_sf(full_geo, coords = c("latitude", "longitude"), crs = 4326)
```

PROBLEM: DATA POINTS MUST BE 
```{r}
tm_basemap() +
tm_shape(bat_geo) + 
  tm_dots(fill = "red") 
